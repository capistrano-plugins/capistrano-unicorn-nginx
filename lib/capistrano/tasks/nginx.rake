  ...
  task :setup_ssl do
    ...
    on roles :web do
      ...
      if fetch(:nginx_upload_local_cert)
        sudo_upload! File.expand_path(fetch(:nginx_ssl_cert_local_path)), nginx_ssl_cert_file
        sudo_upload! File.expand_path(fetch(:nginx_ssl_cert_key_local_path)), nginx_ssl_cert_key_file
      end
